    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def process_batch(batch_texts, batch_start_idx):
            ...
            for j, text in enumerate(batch_texts):
                task = get_embedding_async(session, text)
                tasks.append(task)
...
async def get_embedding_async(session: aiohttp.ClientSession, text: str, max_retries: int = 3) -> Optional[List[float]]:
    """Get single embedding asynchronously with retry logic"""
            ...
            async with session.post(url, json=body, headers=headers, timeout=30) as resp:
                if resp.status == 503:  # Service unavailable
                    if attempt < max_retries - 1:
                        ...
                        return None  # Return None for failed embeddings
